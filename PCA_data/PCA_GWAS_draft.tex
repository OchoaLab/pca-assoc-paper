...
In this scenario we find a clear transition around the ideal number of PCs of $r = 2$ for PCA, below of which performance is poor and above of which performance is satisfactory (\cref{fig:large_sample_size}).
...
The LMM performs as well as PCA with $r = 2$ here without using PCs, with small $\rmsd$ values (though somewhat larger than those of $r=2$) and larger $\auc$ values than PCA's with $r=2$.
However, it is noticeable that there is an increaseing pattern of LMM in terms of rmsd and a decraseing pattern of $\auc$ although slopes are not obvious.
In contrast to previous research results,there is no need to use the combination of PCA adn LMM, which will lead to the worse performance of LMM. 
Thus, in this common scenario where sample sizes are large enough, the PCA approach with enough PCs performs as well as LMM without PCs.
\begin{figure}[bp!]
  \centering
  \includegraphics[width=4in]{boxplot_n_1000.pdf}
  \caption{
    {\bf Evaluation in large sample size admixture scenario.}
    Here there are $n = 1,000$ individuals in the simulation.
    The PCA approach and LMM approach are tested under varying number of PCs ($r \in \{0, ..., 90\}$), alongside the LMM approach , with boxplots for 10 replicates (y-axis) for the distributions of $\rmsd$ (top panel) and $\auc$ (bottom panel).
    Small $\rmsd$ and large $\auc$ correspond with better performance.
	 The $\rmsd$ of LMM has an increasing pattern with the usage of PCs and $\auc$ of LMM has a decreasing pattern over PCs. Hence, the optimal number of PCs is $r=0$ for LMM.
    The ideal number of PCs is $r = 2$ for PCA approach, which results in near zero $\rmsd$ and peak $\auc$, and performs as well as the LMM without PCs.
    PCA with $r < 2$ has incorrect p-values ($\rmsd \gg 0$ cases) and lowest predictive power (small $\auc$).
    Remarkably, PCA remains robust even in extreme $r > 2$ cases, with $\rmsd$ near zero up to $r = 90$ and minimal loss of power as $r$ increases to 90.
	 It can be seen that there are one increasing pattern of rmsd and one decreasing pattern of $\auc$ for LMM. 
	 LMM without PCs seems to be the best choice of LMM in GWAS.
  }
...
Interestingly, we find that the relationship between $\rmsd$ and $r$ is similar under small and large sample sizes for PCA, with ideal near-zero $\rmsd$ distributions for $r \ge 2$.
However, for LMM, the relationship of PCs is different from previous situation. 
When $r \le 60$, the usage of PCs in LMM will lead to an increase in $\rmsd$. 
Moreover, when $r\ge 60$ and $r \le 75$, fluctuation exists in terms of $\auc$.
Lastly, when $r \ge 75$, the $\rmsd$ of LMM will close to 0 which indicates that LMM fits well again. 

On the other hand, we do see a more severe overfitting effect here that results in decreased predictive power: $\auc$ peaks at $r = 2$ as expected, but drops more rapidly as $r$ increases, with performance around $r = 50$ that is as bad as for $r = 0$, and practically zero $\auc$ at $r = 90$ (\cref{fig:small_sample_size}).
Simialrly,the distribution of auc of LMM in the case of small sample size is also different from the case of large sample size.  
The auc of LMM will decrease with the number of PCs used in LMM increases for $r \le 70$.
Notably, when $r \ge 70$, $\auc$ of LMM will begin to increase and will close to the $\auc$ of LMM with 0 PC used for$r \ge 85$.
Hence, in the case of small sample size, we should always be cautious about using PCs as it is easy to result in overfitting in terms of $\auc$ for both PCA and LMM.
Moreover, for LMM, it seems that the optimal choice is still set $r=0$ to avoid overfitting problem.



\begin{figure}[bp!]
  \centering
  \includegraphics[width=4in]{boxplot_n_100.pdf}
  \caption{
    ...
    The pattern for $\rmsd$ in the top panel is similar to the previous figure for PCA. 
    However, here there is a more pronounced drop in $\auc$ values as the number of PCs $r$ increases from $r=2$ to $r=90$.
	The pattern of LMM for $\rmsd$ and $\auc$ are quite different from previous figure. 
	The pattern in the first panel for LMM will increase first with the increase of number of PCs until $r=60$. 
	Then, $\rmsd$ will fluctuate a lot for the number of PCs from $r=60$ to $r=75$.
	After that, it can be seen that rmsd will decrease to 0 immediately when $r \ge 75$ which indicates a good controlling over type one error when enough PCs are used in LMM.
	On the other side,the second pannel for the mixed effect is also different from the pattern in large sample size. 
	The pattern of auc decreases until $r=75$ and then, begins to increase for $r \ge 75$ and $r \le 90$.	
  }
...
In this case $r = 2$ is still the optimal choice for PCA, even though the rank of the genotype matrix is much greater due to the family structure.
...
However, it can be seen that $\rmsd$ for LMM is close although there still exists a decreasing pattern of $\rmsd$ with the increase of PCs used in LMM.'

Additionally, the $\auc$ increases until $r = 2$ is reached (as opposed to $r = 9$ as before), then plateaus with marginal decreases in performance as $r$ goes to 90.
In contrast, the LMM does achieve a near-zero $\rmsd$ and $\auc$ distribution of LMM is always above $\auc$ distribution of PCA.
Therefore, it can be seen that LMM out performs PCA in the case of family structure. 

\begin{figure}[bp!]
  \centering
  \includegraphics[width=4in]{boxplot_n_1000_family.pdf}
  \caption{
    ...
    Unlike previous figures, here $\rmsd$ (top panel) for PCA does not go down to zero as $r$ increases.
	 However,  $\rmsd$ (top panel) for LMM	will close to 0 which indicates a good control in type one error control.
    For this complex relatedness structure $r =2$ is still the optimal number of PCs and performance remains steady for all $r \ge 2$ values tested.
	 In addition to this, for the second panel, $r =2$ is also the best number of PCs for PCA in terms of $\auc$, whereas for LMM, $\auc$ is quite stable regardless of the chang of number of PCs. 
	Overall, it can be seen that LMM outperforms PCA in both  $\rmsd$ and $\auc$ when family structure exists, which agrees with the previous research.
  }
...
One important conclusion of our evaluation is that the PCA approach for genetic association studies is robust to the choice of $r$ (number of PCs), as long as $r$ is large enough, especially when the sample size is large enough.
...
In our simulations that excluded family structure, the optimal choice was $r = 2$,  and we found that even $r = 1$ paid a large penalty in both type-I error control (measured via $\rmsd$) and predictive power ($\auc$; \cref{fig:large_sample_size,fig:small_sample_size}).
...
Unexpectedly, sometimes we see LMM perform much more poorly compared to PCA in our simulation scenarios (middle quartiles in boxplots in \cref{fig:small_sample_size}).
