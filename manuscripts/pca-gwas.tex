...
\title{\Large \textbf{
    Limitations of principal components in quantitative fixed- and mixed-effects genetic association models
  }}
...
\begin{abstract}
  ...
  Moreover, PCA is vastly outperformed by LMM in the family structure simulation, in all of the real human datasets, and the tree simulations.
...
When naive approaches are incorrectly applied to structured populations or family data, association statistics (such as $\chi^2$) become inflated relative to the null expectation, resulting in greater numbers of false positives than expected and loss of power \citep{devlin_genomic_1999, voight_confounding_2005, astle_population_2009}.
Therefore, many specialized approaches have been developed for genetic association in structured data.
Here we focus on extensively evaluating the two most popular approaches: principal components analysis (PCA) and linear mixed-effects models (LMM).
...
%We aim to systematically evaluate the robustness of both the PCA (fixed effects only) and LMM (mixed effects) approaches to the choice of number of PCs, especially in cases where the model is grossly misspecified, in more realistic simulations relevant to today's genetics research.

% this work
In this work, we study the performance of the fixed- (PCA) and mixed- (LMM) effects models for genetic association studies, characterizing their behavior under various numbers of PCs (which are included as fixed covariates in both PCA and LMM models).
...
LMMs greatly outperforms PCA in an admixed family simulation, as expected \citep{patterson_population_2006, price_new_2010}.
...
\begin{table}[hb!]
  ...
  \begin{flushleft} 
    \textsuperscript{a}For family structure, ignores the increase in dimensionality due to 20 generation pedigree structure.
...
\begin{FPfigure}%[H]
  ...
  \caption{
    ...
    \textbf{A.}
    High-admixture scenario, used for both ``large'' and ``small'' sample size simulations.
    \textbf{B.}
    Family structure after 20-generations with admixed founders and assortative mating, shows larger kinship values near diagonal corresponding to siblings, first cousins, and other such close relatives.
    \textbf{C.}
...
The family structure simulation additionally drew a new random pedigree for each replicate [TODO: not true, change?].
...
\begin{figure}[bp!]
  \centering
  \includegraphics[width=4in]{sim-n1000-k10-f0.1-s0.5-g1/measures-illustration.pdf}
  \caption{
    {\bf Illustration of $\rmsd$ and $\auc$ evaluation measures.}
    Three archetypal models (named M1, M2, M3) are used to illustrate the properties of our two complementary measures.
    ...
    M1 has uniformly distributed null p-values as desired (curve overlaps $y=x$ diagonal line).
    M2/M3 have p-values larger/smaller than expected (curve above/below diagonal).
    \textbf{B.}
    The $\rmsd$ (p-value Signed Root Mean Square Deviation) summarizes null p-value accuracy using a scaled Euclidean distance between the points in the observed null p-value distribution and their uniform expectation, with a negative sign if the median is larger than expected (closer to zero is better).
    \textbf{C.}
    Precision-Recall plot assesses classification prediction performance across significance thresholds, without assuming that p-values are accurate (only locus ranks matter).
    M1 performs best, delivering the highest precision values at most recall thresholds.
    M2 performs nearly as well as M1, but precision drops more quickly in the middle of the curve.
    M3 performs worst, with the lowest precision values at every recall threshold.
    \textbf{D.}
...
% PCA, LMM, PCs series
The overall goal is to characterize the performance of two main approaches: ``PCA'' (fixed effects only) and ``LMM'' (mixed effects).
...
The large sample size admixture simulation has $n = 1,000$ individuals, and differs from previous admixture evaluations in featuring a larger number of ancestral populations ($K=10$) and more differentiation ($\Fst = 0.1$ for the admixed individuals).
...
\begin{FPfigure}%[H]
  \centering
  \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{const_herit_loci/rmsd-auc-sim.pdf}
  \caption{
    {\small 
      ...
      \textbf{A.}
      The large sample size simulation has $n = 1,000$ individuals.
      \textbf{B.}
      The small sample size simulation has $n = 100$ individuals, helps illustrate overfitting for large $r$.
      \textbf{C.}
      The family structure simulation has $n = 1,000$ individuals from a family structure simulation with admixed founders and large numbers of pairs of sibling, first cousins, second cousins, etc, from a realistic random 20-generation pedigree.
...
The last of the first three simulations adds a 20-generation family structure to our admixture simulation.
...
\begin{FPfigure}%[H]
  \centering
  \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{const_herit_loci/rmsd-auc-real.pdf}
  \caption{
    {\small 
      {\bf Evaluations in real human genotype datasets.}
      Traits simulated from \textit{fixed effect sizes} model.
      Same setup as \cref{fig:rmsd-auc-sim}, see that for details.
      These datasets strongly favor LMM with $r = 0$ PCs over PCA, resulting in curves that resemble the previous family structure simulation, even though these datasets has no close family members.
      \textbf{A.}
...
\begin{FPfigure}%[H]
  \centering
  \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{const_herit_loci/rmsd-auc-real-sim.pdf}
  \caption{
    {\small 
      {\bf Evaluations in tree simulations fit to human data.}
      Traits simulated from \textit{fixed effect sizes} model.
      Same setup as \cref{fig:rmsd-auc-sim}, see that for details.
      These tree simulations do not explain the large gaps in LMM-PCA performance observed in the real datasets.
      \textbf{A.}
...
\begin{figure}[bp!]
  \centering
  \includegraphics[width=3in]{sum-rmsd-vs-lambda.pdf}
  \caption{
    {\bf Comparison between $\rmsd$ and inflation factor $\lambda$.}
...
We fit this model to the upper portion of the data only ($\lambda > 1$), since it was less noisy and of greater interest, and obtained the curve shown in \cref{fig:rmsd_lambda} with parameters $a = 0.567$ and $b = 0.615$.
...
We consider three admixture simulation scenarios, refered to as (1) large sample size, (2) small sample size, and (3) family structure.
All cases are based on the admixture model described previously \citep{ochoa_fst1, ochoa_estimating_2021}.

The large sample size and family structure scenarios have $n = 1,000$ individuals, while small sample size has $n = 100$.
...
The local kinship matrix measures the pedigree relatedness; in the first generation, everybody is locally unrelated.
...
From the pool of unpaired individuals, one is picked randomly, and it is paired with the nearest individual that is not a second cousin or closer relative (local kinship must be $< 1/4^3$).
If there are individuals that could not be paired (occurs if remaining unpaired individuals are all close relatives), then the current generation is erased and all individuals are paired anew.
After all individuals are paired, two children per pair are created, to maintained a fixed population size.
Children are reordered by the average coordinate of their parents, preserving the original order when there are ties.

This random pedigree was drawn once and shared across replicates.
However, in each replicate the genotypes of the founder population are drawn anew from the admixture model, and genotypes across the generations are selected randomly.
...
The \texttt{popkin} kinship estimates from each dataset (from the first replicate for simulated genotypes; same ones shown in \cref{fig:kinship}) were used to calcuate the eigenvalues.
...
This equation is useful to compare p-values from statistics that have non-$\chi^2$ distributions (fixed-effect associations can use the t-test).
...
The fixed effects (PCA) genetic association was performed using \texttt{plink2} \citep{chang_second-generation_2015}.
...
The mixed effects (LMM) genetic association was performed using GCTA \citep{yang_gcta:_2011}.
...
When running GCTA with large numbers of PCs in the small sample size simulation, we often encountered errors such as ``the information matrix is not invertible'', ``analysis stopped because more than half of the variance components are constrained'', and ``Log-likelihood not converged (stop after 100 iteractions)'' (sic), in which cases $\rmsd$ and $\auc$ were treated as missing;
...
\begin{figure}[bp!]
  \centering
  \includegraphics[width=\textwidth,height=0.85\textheight,keepaspectratio]{rmsd-auc-sim.pdf}
  \caption{
    {\small 
      {\bf Evaluations in admixture simulations.}
...
\begin{figure}[bp!]
  \centering
  \includegraphics[width=\textwidth,height=0.85\textheight,keepaspectratio]{rmsd-auc-real.pdf}
  \caption{
    {\small 
      {\bf Evaluations in real human genotype datasets.}
...
\begin{figure}[bp!]
  \centering
  \includegraphics[width=\textwidth,height=0.85\textheight,keepaspectratio]{rmsd-auc-real-sim.pdf}
  \caption{
    {\small 
      {\bf Evaluations in tree simulations fit to human data.}
