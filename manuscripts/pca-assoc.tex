...
$\auc$ has been used previously to evaluate association models \citep{rakitsch_lasso_2013}, reflects statistical power for calibrated models (see Models and Methods), and in general is a more robust alternative to statistical power for miscalibrated models.
...
\begin{figure}[bp!]
  ...
  \caption{
    ...
    \textbf{D.}
    The $\auc$ (Precision-Recall Area Under the Curve) summarizes classification performance and reflects power (higher is better).
  }
...
The initial population structure due to admixture is preserved across the generations by strongly biasing mating pairs for proximity over the one-dimensional geography, which results in an indirect ancestry-biased assortative mating.
...
Thus, PCA performs well in the admixture simulation, which is explicitly low-dimensional (see Models and Methods), as well as in tree simulations with few nodes with long branches, such as the trees we fit to the real human data, where a low-dimensional approximation suffices.
...
The second measure, $\auc$, quantifies causal locus classification performance of each model and reflects power, while making it possible to rank miscalibrated models fairly.
...
The type I error rate is the proportion of null p-values below a given p-value threshold $t$.
Calibrated p-values result in a type I error rate near $t$, which may be evaluated for significance using a binomial test.
This measure depends on the choice of threshold $t$ and can give rise to misleading results, as a model may appear calibrated for some $t$ but not for others, or may be significantly miscalibrated only for sufficiently large $t$ (due to lack of power for smaller $t$).
In contrast, $\rmsd = 0$ guarantees calibrated type I error rates at all thresholds $t$, while large $|\rmsd|$ indicates incorrect type I errors for at least some thresholds $t$.
...
However, to establish a clear connection to $\auc$, assume calibrated p-values so the distribution under the null hypothesis $H_0$ is uniform: $\Pr( p < t | H_0 ) = t$.
The numbers of true positives, false positives, and false negatives at $t$ are therefore
\begin{align*}
  \text{TP}(t)
  &=
    m \pi_1 F(t)
...
Therefore, precision and recall are
\begin{align*}
  \text{Precision}(t)
  &=
...
\begin{align*}
  ...
  &=
    ...
    .
\end{align*}
Finally, the area under the curve equals
$
\auc
=
...
Now lets consider a simple yet common case in which model $A$ is uniformly more powerful than model $B$, so that their power functions satisfy $F_A(t) \ge F_B(t)$ for every $t$.
It follows that the inverse functions satisfy $F_A^{-1}( \text{Recall} ) \le F_B^{-1}( \text{Recall} )$ for every recall value.
This ensures that the precision of model $A$ is greater or equal than the precision of model $B$ at every recall value, and therefore $\auc$ is greater or equal for $A$ than $B$.
