...
% # commands to separate main text from supplement into separate files
% # split all pages first
% pdfseparate pca-assoc.pdf main-%d.pdf
% # merge back separate sections (main is pages 1-39, supp is pages 40-45)
% pdfunite main-?.pdf main-1?.pdf main-2?.pdf main-3?.pdf main.pdf
% pdfunite main-4?.pdf supp.pdf
% # cleanup
% rm main-*.pdf
...
\begin{abstract}
  ...
  We evaluate PCA and LMM both varying number of PCs in new realistic genotype and complex trait simulations including admixed families, trees, and real multiethnic human genotype datasets (1000 Genomes, Human Genome Diversity Panel, Human Origins) with simulated traits.
  We find that LMM without PCs always performs best, with the largest effects in the family simulation and real human datasets.
  The large gaps in PCA-LMM performance on human datasets is driven by the high-dimensional effect of large numbers of distant relatives more than the smaller number of highly related pairs.
  While PCA was known to fail on family data, we report strong effects of family relatedness in several genetically diverse human datasets, not avoided by pruning highly related individual pairs.
...
%For simplicity, non-genetic covariates will be omitted from the PCA and LMM models below, but are trivial to include without changing any of our theoretical results.
...
Nevertheless, in PCA and LMM these biased estimates perform as well as unbiased ones \citep{hou_genetic_2022}.
...
Simulations optionally included multiple environment group effects, as follows.
...
\begin{figure}[hp]
  % \begin{FPfigure}%[H]
  \centering
  \includegraphics[width=\textwidth,height=0.75\textheight,keepaspectratio]{kinship.pdf}
  \caption{
...
\begin{table}[b!]
  ...
  \csvreader[
  tabular = lcc|crl|rclc,
  separator = tab,
  table head =
  % header row 1
  \toprule & & & \multicolumn{3}{c|}{LMM $r=0$ vs best $r$} & \multicolumn{4}{c}{PCA vs LMM $r=0$} \\
  % header row 2
  Dataset & Metric & {Trait\textsuperscript{a}} & {Cal.\textsuperscript{b}} & {Best $r$\textsuperscript{c}} & {P-value\textsuperscript{d}} & {Best $r$\textsuperscript{c}} & {Cal.\textsuperscript{b}} & {P-value\textsuperscript{d}} & {Best model\textsuperscript{e}} \\\midrule,
  late after last line = \\\bottomrule
...
The gap between LMM and PCA was reduced in these evaluations, but LMM with $r=0$ again significantly outperforms or ties PCA in all cases (\cref{tab:human_sum_pcs_h3}).

Lastly, we simulated traits with both a low heritability ($h^2 = 0.3$ again) and large environment effects (total $\sigma_\eta^2 = 0.5$) strongly correlated to the low-dimensional population structure (see Methods).
For that reason, PCs may be expected to perform better in this setting (in either PCA or LMM).
However, we find that both PCA and LMM (even without PCs) increase their $\auc$ values compared to the low-heritability evaluations, and there is only a marginal advantage of LMM with a few PCs versus LMM without PCs (\cref{fig:rmsd-auc-sim-fes-env,fig:rmsd-auc-real-fes-env,fig:king_cutoff-env,fig:rmsd-auc-sim-rc-env,fig:rmsd-auc-real-rc-env,tab:human_sum_pcs_env}).
Oh the other hand, p-value calibration is poorer for all models under environment effects.
Interestingly, on RC traits only, here PCA significantly outperforms LMM in the three real human datasets (\cref{tab:human_sum_pcs_env}), the only cases in all of our evaluations where this is observed.
For comparison, we also evaluate an oracle LMM without PCs but with the finest group labels, the same used to simulate environment, as fixed categorical covariates (``LMM lab.''), and see a much larger gain of performance compared to LMM with PCs.
In particular, LMM with labels always has significantly better or tied $\auc$ than either LMM or PCA without such labels (\cref{fig:rmsd-auc-sim-fes-env,fig:rmsd-auc-real-fes-env,fig:king_cutoff-env,fig:rmsd-auc-sim-rc-env,fig:rmsd-auc-real-rc-env,tab:human_sum_pcs_env}).
However, LMM with labels is typically more poorly calibrated than LMM or PCA without labels, which may be since these numerous labels are inappropriately modeled as fixed rather than random effects.
Overall, we find that association studies with correlated environment and genetic effects remain a challenge for PCA and LMM, and that in this case addition of PCs to an LMM improves performance only marginally.
...
Newer approaches not tested in this work have made LMMs more scalable and applicable to biobank-scale data \citep{loh_efficient_2015, zhou_efficiently_2018}, so one clear next step is carefully evaluating these approaches in simulations with larger sample sizes.
...
% Supplemental data include five figures and one table.
...
\begin{figure}[hp]
  ...
  \caption{
    {\bf Evaluation in real datasets excluding 4th degree relatives, low heritability.}
...
\begin{figure}[hp]
  ...
  \caption{
    {\bf Evaluation in real datasets excluding 4th degree relatives, environment.}
    Traits simulated with environment effects, otherwise the same as \cref{fig:king_cutoff-h3}.
  }
...
\begin{table}[b!]
  ...
  Dataset & Metric & {Trait\textsuperscript{a}} & {Cal.\textsuperscript{b}} & {Best $r$\textsuperscript{c}} & {P-value\textsuperscript{d}} & {Best $r$\textsuperscript{c}} & {Cal.\textsuperscript{b}} & {P-value\textsuperscript{d}} & {Best model\textsuperscript{e}} \\\midrule,
  late after last line = \\\bottomrule
  ]{../data/m_causal_fac-27/h0.3/stats.txt}{}{\csvlinetotablerow}
  \begin{flushleft}
    \textsuperscript{a}FES: Fixed Effect Sizes, RC: Random Coefficients.\\
    \textsuperscript{b}Calibrated: whether mean $|\rmsd| < 0.01$.\\
    \textsuperscript{c}Value of $r$ (number of PCs) with minimum mean $|\rmsd|$ or maximum mean $\auc$.\\
...
\begin{table}[b!]
  ...
  \csvreader[
  tabular = lcc|crl|rclc|clc,
  separator = tab,
  table head =
  % header row 1
  \toprule & & & \multicolumn{3}{c|}{LMM $r=0$ vs best $r$} & \multicolumn{4}{c|}{PCA vs LMM $r=0$} & \multicolumn{3}{c}{LMM lab. vs PCA/LMM} \\
  % header row 2
  Dataset & Metric & {Trait\textsuperscript{a}} & {Cal.\textsuperscript{b}} & {$r$\textsuperscript{c}} & {P-value\textsuperscript{d}} & {$r$\textsuperscript{c}} & {Cal.\textsuperscript{b}} & {P-value\textsuperscript{d}} & {Best\textsuperscript{e}} & {Cal.\textsuperscript{b}} & {P-value\textsuperscript{d}} & {Best\textsuperscript{e}} \\\midrule,
  late after last line = \\\bottomrule
  ]{../data/m_causal_fac-27/h0.3/env0.3-0.2/stats.txt}{}{\csvlinetotablerow}
  \begin{flushleft}
    \textsuperscript{a}FES: Fixed Effect Sizes, RC: Random Coefficients.\\
    \textsuperscript{b}Calibrated: whether mean $|\rmsd| < 0.01$.\\
    \textsuperscript{c}Value of $r$ (number of PCs) with minimum mean $|\rmsd|$ or maximum mean $\auc$.\\
