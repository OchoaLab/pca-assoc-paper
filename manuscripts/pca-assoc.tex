...
However, we find that both PCA and LMM (even without PCs) increase their $\auc$ values compared to the low-heritability evaluations (\cref{fig:king_cutoff-env} shows representative numbers of PCs that performed optimally or nearly so in individual simulations shown in \cref{fig:rmsd-auc-sim-fes-env,fig:rmsd-auc-real-fes-env,fig:rmsd-auc-sim-rc-env,fig:rmsd-auc-real-rc-env}).
On the other hand, p-value calibration ($\rmsd$) is poorer for all models under environment effects.
